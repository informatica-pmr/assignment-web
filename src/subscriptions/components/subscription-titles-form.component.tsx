...
>((_props, ref) => {
  ...
  useEffect(() => {
    ...
    setSubscriptionTitles(titles
      .sort((a, b) => a.order! - b.order!)
      .map((t) => ({ titleId: t.titleId, value: 0 })));
        ...
        {points.map((p) => {
          ...
          return (
            <tr key={title.titleId}>
              ...
              <td>
                <input type="text" className="form-control form-control-sm" value={formatter.decimal(subscriptionTitles.find((st) => st.titleId === title.titleId)?.value ?? 0)} 
                onChange={(e) => {
