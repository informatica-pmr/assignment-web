...
} from "../contexts/subscriptions-filters.context";
...
export const SubscriptionsProvider = ({ children }: SubscriptionsProviderProps) => {
  ...
  const createSubscription = useCallback(async (createSubscriptionDTO: CreateSubscriptionsInputDTO) => {
    try {
  ...
  const updateSubscription = useCallback(async (id: string, updateSubscriptionDTO: UpdateSubscriptionsInputDTO) => {
    try {
      await fetch.put<UpdateSubscriptionsInputDTO>(
        id,
