...
} from "../contexts/preferences-filters.context";
...
export const PreferencesProvider = ({ children }: PreferencesProviderProps) => {
  ...
  const createPreference = useCallback(async (createPreferenceDTO: CreatePreferencesInputDTO) => {
    try {
      await fetch.post<CreatePreferencesOutputDTO, CreatePreferencesInputDTO>(
        createPreferenceDTO
      );
  ...
  const updatePreference = useCallback(async (id: string, updatePreferenceDTO: UpdatePreferencesInputDTO) => {
    try {
      await fetch.put<UpdatePreferencesInputDTO>(
        id,
        updatePreferenceDTO
      );
